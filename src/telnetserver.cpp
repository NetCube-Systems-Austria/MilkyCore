#include "httpserver.h"
  ...
  void MilkyTelnetServer::registerUserRequestHandler(uint8_t (*handler)(String, String, EthernetClient)) {
    userRequestHandlers.push_back(handler);
  }

  void MilkyTelnetServer::unregisterUserRequestHandler(uint8_t (*handler)(String, String, EthernetClient)) {
  ...
  void MilkyTelnetServer::handleCommand(String command, EthernetClient client) {
    ...
    if (!action.equals("help")) {
      for ( list<UserRequestHandler>::iterator handler = userRequestHandlers.begin(); handler != userRequestHandlers.end(); handler++ ) {
        if ((*handler)(action, args, client) == 0) return; //If the request was handeled, return.
      }
      ...
      if (action.equals("exit")) {
        ...
        client.println("===============================");
        client.print("      Hardware Address: "); client.println(NetworkHelper.macAddressToString());
      ...
      //Let every handler print their help page
      for ( list<UserRequestHandler>::iterator handler = userRequestHandlers.begin(); handler != userRequestHandlers.end(); handler++ ) {
        (*handler)("help", "", client);
      }
    } 
